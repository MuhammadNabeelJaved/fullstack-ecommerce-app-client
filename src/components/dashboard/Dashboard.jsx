  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium ${isDarkMode ? 'bg-indigo-900/40 text-indigo-300' : 'bg-indigo-50 text-indigo-700'}`}>
      ...
      >
        <div className="absolute top-0 right-0 w-64 h-64 -mt-12 -mr-12 opacity-20">
          <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
            <path fill="#FFFFFF" d="M47.9,-57.2C59.8,-47.8,66.2,-30.6,68.6,-13.5C71,3.6,69.4,20.7,60.9,33.4C52.5,46.1,37.2,54.3,21.2,59.9C5.2,65.4,-11.6,68.3,-29.4,65C-47.3,61.6,-66.1,52.1,-72.9,37.2C-79.7,22.3,-74.5,2.1,-68.2,-15.7C-61.9,-33.5,-54.5,-48.7,-42.6,-58C-30.6,-67.3,-15.3,-70.7,1.2,-72.2C17.7,-73.6,35.9,-66.6,47.9,-57.2Z" transform="translate(100 100)" />
          </svg>
        </div>
        <div className="relative">
          <h2 className="text-2xl font-bold text-white mb-2">Welcome back, John!</h2>
          <p className="text-indigo-100 mb-6 max-w-xl">Here's an overview of your recent activity and stats for March 2024. You have 2 pending orders and 1 delivery scheduled for tomorrow.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <h2 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Recent Orders</h2>
            <Link 
              to="/dashboard/orders" 
              ...
              <div key={order.id} className="p-5 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{order.id}</p>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{order.date}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 sm:ml-auto">
                    <div className="text-right">
                      <p className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{order.total}</p>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{order.items} items</p>
                    </div>
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-medium inline-flex items-center
                        ${
                          order.status === 'Delivered'
                            ...
                            ? isDarkMode ? 'bg-amber-900/40 text-amber-400' : 'bg-amber-100 text-amber-800'
                            : isDarkMode ? 'bg-blue-900/40 text-blue-400' : 'bg-blue-100 text-blue-800'
                        }`}
                    >
        ...
        >
          ...
          {upcomingDeliveries.length > 0 ? (
              ...
              {upcomingDeliveries.map((delivery) => (
                <div key={delivery.id} className="p-5">
                  ...
                  <div className="mt-3 flex items-center">
                    ...
                    <span className={`text-sm ml-1.5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      {delivery.status}
                    </span>
                  </div>
                </div>
            ...
            <div className="p-6 text-center">
              <RiTruckLine className={`w-12 h-12 mx-auto ${isDarkMode ? 'text-gray-600' : 'text-gray-300'}`} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <p className="text-indigo-200 mt-1 relative">Check your order status</p>
          <Link to="/dashboard/orders" className="absolute inset-0" aria-label="Track Order">
        ...
        >
          ...
          <Link to="/dashboard/wishlist" className="absolute inset-0" aria-label="View Wishlist">
        ...
        >
          ...
          <p className="text-emerald-200 mt-1 relative">Manage your payment options</p>
          <Link to="/dashboard/profile" className="absolute inset-0" aria-label="Payment Methods">
